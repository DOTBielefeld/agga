...
class Scenario:
    ...
    def read_scenario_files(self, scenario):
        ...
        if "paramfile" in scenario:
            scenario["parameter"], scenario["no_goods"], scenario["conditionals"] = get_ta_arguments_from_pcs(f'/{scenario["paramfile"]}')
        else:
            raise ValueError("Please provide a file with the target algorithm parameters")

        if "instance_file" in scenario:
            scenario["instance_set"] = read_instance_paths(f'/{scenario["instance_file"]}')
        else:
            raise ValueError("Please provide a file with the training instances")

        if "test_instance_file" in scenario:
            scenario["test_instances"] = read_instance_paths(f'/{scenario["test_instance_file"]}')
        else:
